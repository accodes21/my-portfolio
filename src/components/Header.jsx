...
const Header = () => {
  return (
    ...
    <motion.header {...slideAnimation('down')} className='w-full'>
        <nav className='fixed z-10 cursor-pointer flex-row items-center  overflow-hidden rounded-full border px-1 py-4 shadow-xl backdrop-blur-sm backdrop-filter border-[#f1cf29]/60 bg-black bg-opacity-60 lg:left-1/2 lg:flex lg:-translate-x-1/2 lg:transform mt-6 top-8"'>
            <motion.ul {...introHeaderVariants('left')} className='flex justify-end pr-4 text-lg'>
                ...
                <Link href={'#contact'} className='ml-4 text-white hover:text-yellow-400 transition-all'>Contact</Link>
            </motion.ul>
        </nav>
    </motion.header>
    </AnimatePresence>
